<template>
        <div class="card lg:w-[50rem] md:w-full bg-base-100 shadow-xl my-10">
            <div class="card-body">
                <p class="text-lg py-7">
                    How satisfied are you with your brand visibility?
                </p>
                <div class="flex justify-center items-center lg:gap-10 md:gap-7">
                    <div>Not at all likely</div>
                    <div v-for="n in 5" class="flex lg:gap-10 md:gap-7">
                        <div class="flex flex-col gap-2">
                            <span>{{n}}</span>
                            <input type="radio" name="radio-12" class="radio" @input="radioInput12(n)" />
                        </div>
                    </div>
                    <div>Extremely likely</div>
                </div>
            </div>
          </div>
</template>

<script>
export default {
  name: "QuestionForm",
  methods: {
    radioInput12(value) {
      console.log("VALUE", value)
      this.$store.commit('newStore/set_val_12', value)
    },
  }
}


</script>
